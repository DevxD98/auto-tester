...
export class HuggingFaceProvider implements AiProvider {
  ...
  async generateTests(input: { pageModels: PageModel[]; requirementsText?: string; maxTests?: number }): Promise<TestSpec[]> {
    ...
    const res = await fetch(`https://api-inference.huggingface.co/models/${encodeURIComponent(this.cfg.model)}` , {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        parameters: { max_new_tokens: 800, temperature: this.cfg.temperature },
        options: { wait_for_model: true, use_cache: true }
      })
    });
    if (!res.ok) return [];
    const data = await res.json();
    const text: string = Array.isArray(data) ? (data[0]?.generated_text || '') : (data?.generated_text || JSON.stringify(data));
    ...
    const schema = `JSON schema (informal):
{
  "tests": [
    ...
    const user = {
      instructions: {
        maxTests,
        focus: ['forms','auth','navigation','filters','edge cases']
      },
